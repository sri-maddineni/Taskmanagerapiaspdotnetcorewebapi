...
namespace Mongocrud.DataAccessLayer
{
    public class CrudDL : ICrudDL
	{
		...
		private readonly IMongoCollection<GetAllTasksRequest> _mongoCollection;
		...
		public CrudDL(IConfiguration configuration)
		{
			...
			_mongoCollection = _MongoDatabase.GetCollection<GetAllTasksRequest>(_configuration["Database:CollectionName"]);



		}

		public async Task<GetAllTasksResponse> GetAllTasks()
		{
			GetAllTasksResponse res = new GetAllTasksResponse();
			res.success = true;
			res.message = "Success added!";

			try
			{
				res.data = new List<GetAllTasksRequest>();
				res.data = await _mongoCollection.Find(x => true).Limit(10).ToListAsync();
		...
		public async Task<GetAllTasksResponse> DeleteAllTasks()
		{
			GetAllTasksResponse res = new GetAllTasksResponse();
			res.success = true;
			res.message = "Success added!";
